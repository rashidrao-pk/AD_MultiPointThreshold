...
def create_video_custom(
    ...
    images = sorted(
        [img for img in os.listdir(folder_path) if img.endswith(".png") or img.endswith(".jpg")],
...
def create_video_from_frames(
    ...
    folder_path=None,
    fps=5,
    filter_frame_name=None,
    verbose=False,
    interval=10,
    repeat=10,
    save_path_type="cloud",
):
    """
    Create a video from a folder of images
    :param folder_path: Path to the folder containing the images
    ...
    if video_for == "test":
        ...
        allowed_images = sorted(
            [
                x
                for x in images
                if filter_frame_name in x
                and int(x.split(".")[0].split("_")[last_id]) % interval == 0
            ],
            key=natural_sort_key,
        )
...
class LoadMaskedImage:
    ...
    def __init__(
        ...
        mask_path = os.path.join(
            paths.mask_dir,
            f"{paths.dataset_type}_{mask_image_name}_{subgroup}_{subgroup_mask}_ext.png",
        )
        print(mask_path)
    ...
    def __call__(self, image):
        ...
        return Image.composite(
            image.convert("RGB"), Image.new("RGB", image.size, self.fill_color), self.mask
        )
...
class MaskedCrop:
    ...
    def uncrop(self, image_np):
        ...
        assert (
            image_np.shape[0] == self.cropped_mask.shape[0]
            and image_np.shape[1] == self.cropped_mask.shape[1]
        ...
        orig_image_np = np.zeros(
            (self.mask.shape[0], self.mask.shape[1], no_channels), dtype=image_np.dtype
        )
...
def get_data_loaders(
    ...
    if paths.dataset_type == "fronttop":
        ...
        mask_path = (
            f"{paths.dataset_type}_{mask_image_name}_{subgroup}_{params.subgroup_mask}_ext.png"
        )
    else:
    ...
    if aug_type == "min":
        ...
        transform_train = transforms.Compose(
            [
                MaskedCrop(
                    subgroup=params.subgroup, mask=component_mask.astype("bool")
                ),  # Just the cropping part
        ...
        transform_train = transforms.Compose(
            [
                LoadMaskedImage(
                    ...
                    (orig_image_size),
                    subgroup=params.subgroup,
                    subgroup_mask=params.subgroup_mask,
                    map_coor=params.map_coor,
    ...
    if reorder_test_classes:
        if reorder_test_classes_opp:
            ...
            test_dataset.samples = [
                (path, new_class_to_idx[label]) for path, label in test_dataset.samples
            ]
        else:
            original_class_to_idx = test_dataset.class_to_idx
    ...
    test_loader = DataLoader(
        test_dataset, batch_size=batch_size, shuffle=True, num_workers=0, pin_memory=pin_memory
    )
...
def get_data_loaders_for_contour(
    paths,
    params,  #  train_dir,test_dir,
    root_dir=None,
    subgroup=None,
...
def get_data_loaders_for_contour_and_gt(
    paths,
    params,  #  train_dir,test_dir,
    root_dir=None,
    subgroup=None,
    ...
    if image_to_safety_area:
        ...
        if paths.dataset_type == "fronttop":
            ...
            mask_path = (
                f"{paths.dataset_type}_{mask_image_name}_{subgroup}_{params.subgroup_mask}.png"
            )
        if verbose:
            print(f"using {subgroup} group and {mask_path}")
    ...
    if image_to_safety_area:
        transform = transforms.Compose(
            [
                MaskedCrop(
                    subgroup=subgroup, mask=component_mask.astype("bool")
                ),  # Just the cropping part
                transforms.Resize(params.target_size),
                transforms.ToTensor(),
                transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),
    ...
    dataset_segmentation_maps = datasets.ImageFolder(
        root=segmentation_masks_dir, transform=transform
    )
...
##################################################################################################
def get_data_loaders_from_preprocessed(
    ...
    train_dataset = datasets.ImageFolder(
        root=train_dir_processed_subgroup, transform=transform_train
    )
    if verbose:
...
def get_test_loaders_from_processed_data_new(
    ...
    if reorder_test_classes:
        ...
        test_dataset.samples = [
            (path, new_class_to_idx[idx_to_class[label]]) for path, label in test_dataset.samples
        ]
...
###########################################################################################################
def get_test_loaders_from_processed_data(
    paths,
    params,
    data_type="test",
    sel_type="min-mask",
...
#################################################################################################
def show_and_save(
    file_name, real_batch, paths, data_type="tuple", img_count=2, destroy_fig=True, fontsize=14
):
    ...
    plt.title(
        file_name, fontweight="bold", fontsize=fontsize
    )  # y parameter controls the vertical position
...
def plot_test_images(test_loader, paths):
    ...
    real_batch = (
        real_batch.cpu().numpy().transpose(0, 2, 3, 1) * 0.5 + 0.5
    )  # (B, C, H, W) → (B, H, W, C)
...
def get_colormap(mode=1):
    ...
    if mode == 1:
        # TUNED Colormap
        colormap_anomaly_map = LinearSegmentedColormap.from_list(
            "colormap_anomaly_map",
            [(0.0, "white"), (0.9 / 2, "skyblue"), (0.91 / 2, "violet"), (1.0, "red")],
        )
    else:
        colormap_anomaly_mapv1 = LinearSegmentedColormap.from_list(
...
def plot_test_single(
    ...
    if plot_anomaly_scores:
        ...
        axes[2].text(
            10, 15, f"Score: {anomaly_score:.2f}", color=fontcolor, fontsize=fontsize, weight="bold"
        )
    if plot_suptitle:
    ...
    if save_fig:
        ...
        plt.savefig(
            f"{save_path}/{ttl}.png", transparent=transparent_fig, dpi=dpi, bbox_inches="tight"
        )
    if destroy_fig:
...
def get_anomaly_score(data_batch, recon_batch):
    ...
    assert data_batch.shape == recon_batch.shape, (
        f"Shape mismatch: {recon_batch.shape} vs {data_batch.shape}"
    )
...
def plot_images(
    ...
    for i_id_i, (i_id, i) in enumerate(zip(ttl[0:n_images], range(n_images))):
        ...
        if plot_anomaly_scores:
            ...
            axes[i * 3 + 2].text(
                10, 15, f"Score: {score:.2f}", color=fontcolor, fontsize=fontsize - 2, weight="bold"
            )
...
def plot_images_tracking(
    ...
    fig, axes = plt.subplots(
        nrows=1, ncols=3 * 3, figsize=(9, 1.5)
    )  # 3 images (original, reconstructed, anomaly map)
    ...
    for c, (original, reconstructed, title_prefix) in enumerate(categories):
        ...
        axes[base_idx + 2].imshow(
            diff_img.numpy(), cmap="Reds" if cmap is None else colormap_anomaly_map, vmin=0, vmax=2
        )
...
def plot_loss_sep(
    ...
    ax.plot([item["recon_loss"] for item in loss_history], label="Reconstruction Loss", c="blue")
    ax.plot([item["kl_loss"] for item in loss_history], label="KL Loss", c="teal")
    ax.plot([item["beta_kl_loss"] for item in loss_history], label="beta * KL Loss", c="darkturquoise")
    ax.plot([item["gan_loss"] for item in loss_history], label="GAN Loss", c="green")
    ax.plot([item["beta_gan_loss"] for item in loss_history], label="beta * GAN Loss", c="limegreen")
    ax.plot([item["vae_loss"] for item in loss_history], label="VAE Loss", c="indianred")
    ax.plot([item["annealing_lambda"] for item in loss_history], label="Annealing", c="lightgray")
    ax.set_xlabel("Epoch")
    ...
    ax.scatter(
        ...
        [l["dis_acc"] for l in loss_history],
        label="Accuracy(Dis)",
        c="orangered",
        alpha=0.5,
        marker="+",
    ...
    ax.scatter(
        range(len(loss_history)),
        [l["dis_F1"] for l in loss_history],
        label="F1(Dis)",
        c="royalblue",
        alpha=0.5,
        marker="x",
    ...
    if plot_type == 3:
        ...
        ax.scatter(
            range(len(loss_history)),
            [l["disc_loss"] for l in loss_history],
    ...
    if plot_long_header:
        ...
        values = [
            ...
            paths.dataset_type,
            params.subgroup,
            f"{len(loss_history)} ({params.epochs})",
        ...
        header_row = (
            "| " + " | ".join(h.ljust(w) for h, w in zip(column_headers, column_widths)) + " |"
        )
...
def plot_losses(
    ...
    if kl_usuage:
        kl_text = "USING KL loss"
    else:
        kl_text = "WITHOUT KL loss"
    fig = plt.figure(figsize=(12, 6))

    plt.plot(
        [l["recon_loss"] for l in loss_history],
        label="Recon Loss",
        linestyle="dashed",
        color="blue",
        alpha=0.7,
    ...
    plt.plot(
        [l["kl_loss"] for l in loss_history],
        label="KL Loss",
        linestyle="dashed",
        color="orange",
        alpha=0.7,
    ...
    plt.plot(
        [l["gan_loss"] for l in loss_history],
        label="GAN Loss",
        linestyle="dashed",
        color="green",
        alpha=0.7,
    ...
    if plot_long_header:
        # Define column headers and widths
        column_headers = ["Epochs", "Beta KL", "Beta GAN", "Recon Loss", "Adv Loss"]
        ...
        values = [
            ...
            params.beta_kl,
            params.beta_gan,
            params.reconstruction_loss_fn,
            params.adversarial_loss_fn,
        ]
        ...
        for gaussian_weights in gaussian_weights_ls:
            for sigma in sigma_ls:
                for agg_func in aggregation_function_ls:
                    name = (
                        f"w{win_size}-s{sigma}-f-{agg_func}"
                        if gaussian_weights
                        else f"w{win_size}-s{sigma}-{agg_func}"
                    )
                    ssim_params[idx] = {
                        "name": name,
                        "win_size": win_size,
...
class ComputeDifferences:
    ...
    def compute(self, type="l1", params=None):
        ...
        if type == "l1":
            ...
            return self.get_ssim_dissimilarity_batch(
                ssim_type=type, params=params, supress_output=True
            )
        else:
    ...
    def get_l2_difference(self, supress_output=False):
        ...
        diff_score = (
            torch.norm(self.data_batch - self.recon_batch, p=2, dim=(1, 2, 3)).cpu().numpy()
        )  # (B,)
        if supress_output:
            diff_map = diff_map.cpu().numpy()
            diff_map = np.moveaxis(diff_map, 1, 3)  # (B, C, H, W) → (B, H, W, C)
    ...
    def get_gaussian_difference(self, params=None):
        ...
        gaus_score, gaus_image = get_anomaly_score(
            self.data_batch, self.recon_batch, offset=offset, sigma=sigma, quant=quant, debug=False
        )
    ...
    def get_ssim_dissimilarity_batch(self, ssim_type="ssim", params=None, supress_output=False):
        ...
        #########################################################################
        if params is not None:
            win_size, gaussian_weights, sigma = (
                params["win_size"],
                params["gaussian_weights"],
                params["sigma"],
            )
        else:
    ...
    def plot_differences(
        ...
        if type is None:
            ...
            type == "ssim" or type == "ssim_all" or type == "ssim_d" or type == "l1" or type == "l2"
        ):
            if diff_img.ndim == 4:
                diff_img = np.moveaxis(diff_img[id], 0, 2)  # Convert (C, H, W) → (H, W, C)
        ...
        rect = patches.Rectangle(
            ...
            edgecolor=None,
            facecolor="white",
            alpha=0.8,  # White background with transparency
        )
        ax[2].add_patch(rect)
    ...
    def plot_differences_all(
        ...
        # print(diff_img.shape)
        fig, axes = plt.subplots(
            nrows=1, ncols=len(diff_img_ls) + 2, figsize=figsize, sharex=sharex, sharey=sharey
        )
        ax = axes.ravel()
        ...
        # Handle different formats of diff_img (either PyTorch tensor or NumPy array)
        for idx, (diff_metadata, diff_data, score_data) in enumerate(diff_img_ls):
            diff_img = diff_data["image"]
            diff_img_key = diff_metadata["key"]
            diff_scores = score_data["score"]
...
def get_explore_latent_space(
    train_loader, Enc, Dec, data_type="full", data_part_type="train", device="cpu"
):
    ...
    data, labels, filenames_test = get_data_latent_inspection(
        train_loader, data_type=data_type, data_part_type=data_part_type
    )
    if device == "cuda":
        data = data.to(device)
...
def get_explore_latent_space_batched(
    ...
    if method == "pca":
        if use_incremental_pca:
            reducer = IncrementalPCA(n_components=n_components, batch_size=ipca_batch_size)
...
def plot_latent(
    ...
    for cls in np.unique(labels_train):
        ...
        # color = "#0088FFFF"
        plt.scatter(
            latent_space[mask_train, 0],
            latent_space[mask_train, 1],
...
def plot_latent_3d_plotly_custom(
        ...
        for cls in np.unique(labels_train):
            ...
            fig.add_trace(
                go.Scatter3d(
                    ...
                    mode="markers",
                    marker=dict(size=4, color=color, symbol=marker_symbol, opacity=0.7),
        ...
        for cls in np.unique(labels_test):
            ...
            fig.add_trace(
                go.Scatter3d(
                    ...
                    mode="markers",
                    marker=dict(size=4, color=color, symbol=marker_symbol, opacity=0.8),
    ...
    fig.update_layout(
        title=title,
        scene=dict(
            xaxis_title="Latent Dim 1", yaxis_title="Latent Dim 2", zaxis_title="Latent Dim 3"
        ),
...
def get_contoured_image(
    ...
    cv2.drawContours(
        contoured_image, contours, -1, contour_color, thickness
    )  # (0, 255, 255) corresponds to yellow, 2 is the thickness
    if verbose:
        print(mask_path)
...
def get_parameters_by_experiment(params, verbose=False):
    ...
    if params.exp_type == "E1":
        ...
        params.learning_rate_dis = (
            0.0001  # should be 10x smaller that enc_dec to improve convergence
        )
        # -----------------------------------------
...
def get_dataset_version(
    ...
    if params.subgroup == "full":
        ...
        paths.train_dir_processed_subgroup = os.path.join(
            paths.path_dataset_selected, paths.train_dirn_processed
        )
    else:
        ...
        paths.train_dir_processed = os.path.join(
            paths.path_dataset_selected, paths.train_dirn_processed
        )
        paths.test_dir_processed = os.path.join(
            paths.path_dataset_selected, paths.test_sel_dirn_processed
        )
        ...
        paths.train_dir_processed_subgroup = os.path.join(
            paths.train_dir_processed, params.subgroup
        )
...
def load_tuned_threshold(paths, params, component=None, verbose=False):
    ...
    if os.path.exists(csv_path):
        ...
        # df.head()
        best_anom_score, best_anomaly_score_index, best_threshold = (
            sorted_df.iloc[0].name,
            sorted_df.iloc[0].original_position,
            sorted_df.iloc[0].Threshold,
        )
...
def plot_anomalymaps(
    ...
    norm_score,
    threshold_cmap,
    threshold_cmap_unexpected,
    selected_threshold,
    fontsize=10,
    destroy_figs=False,
):
    """Plot and save a single anomaly map."""

    if selected_threshold <= 0:
        raise ValueError(f"selected_threshold must be positive, got {selected_threshold}")

    cmap = threshold_cmap if norm_score < 1 else threshold_cmap_unexpected
    ...
    ax.imshow(
        ...
        cmap=cmap,
        vmin=0,
        vmax=2 * selected_threshold,
    )

    ax.set_title(
        f"{component} - N-Score: {normscore_text}",
        color=color_score,
        fontsize=fontsize,
    )
    ax.set_xticks([])
    ...
    output_dir = Path(video_savepath_part)
    output_dir.mkdir(parents=True, exist_ok=True)

    output_path = output_dir / f"{component}_anommap_{idx}.png"
...
def plot_anomalymaps(
    ...
    plt.title(f"{component} - N-Score: {normscore_text}", color=color_score, fontsize=fontsize)
    plt.xticks([])
    plt.yticks([])

    plt.savefig(
        f"{video_savepath_part}/{component}_anommap_{idx}.png", dpi=150, bbox_inches="tight"
    )
    if destroy_figs:
        plt.close(fig)
    plt.show()
...
def plot_component_boundary(
    full_anomaly_map_np, base_mask, norm_score, ax, threshold_cmap, verbose=False
):
...
def plot_comparison(
    ...
    for i, component in enumerate(components_ls):
        ...
        thres_text = rf"$\mathbf{{\tau={component_threshold[component]:.4f}}}$"

        ax.imshow(
            comp_map_list[component],
            cmap=threshold_cmap,
            alpha=0.7,
            vmin=0,
            vmax=2 * selected_threshold,
        ...
        if verbose:
            print(
                component,
                "[",
                np.min(comp_map_list[component]),
                "~",
                np.max(comp_map_list[component]),
                "]",
            )
        ax.set_title(
            f"{component} - {thres_text}\nN-Score: {normscore_text}×\n score: {batch_scores_component[component]:.4f}",
            color=color_score,
            fontsize=10,
        )
        ax.axis("off")
...
def plot_final_results_v2(
    ...
    for component in components_ls:
        ...
        if verbose and idx == 0:
            print(
                f"{component:<10}: (tau: {component_threshold[component]:<10.3f})(Score: {norm_score:<10.3f} {'UNEXPECTED' if is_anomalous else 'normal'})"
            )
...
def segments_to_color(real_batch_segments, verbose=True):
    ...
    colors_count_dict = {tuple(color): int(count) for color, count in zip(colors_found, counts)}
    if verbose:
...
def plot_found_color(colors_found, counts, rounded_colors, verbose=True):
    ...
    if verbose:
        ...
        for color, count in zip(colors_found, counts):
            print(
                f"RGB: ({color[0]:3d}, {color[1]:3d}, {color[2]:3d}) | Pixels: {count:7d} | "
...
def get_combined_mask(
    ...
    disallowed_color=None,
    components_ls=None,
    combine_types=["PLeft", "PRight"],
    plot_=True,
    plot_found_color_=False,
    plot_title=True,
    fixed_contour_color_val=None,
    ...
    if check_anomalous:
        colors_found, counts, rounded_colors, colors_count_dict = segments_to_color(
            seg_np, verbose=False
        )
    ...
    for component in components_ls:
        if component in combine_types:
            ...
            contoured_image, component_mask = get_contoured_image(
                overlay_image,
                paths,
                subgroup,
                mask_image_name=mask_image_name,
                contour_color=(0, 255, 0),
                thickness=4,  # Bright green
            )
            ...
            colors_found_component_level, counts, rounded_colors, colors_count_dict = (
                segments_to_color(cropped_segmentation, verbose=verbose)
            )
            # is_anomalous = True if disallowed_color in colors_found_component_level else False
            is_anomalous = (
                True
                if np.any(np.all(colors_found_component_level == disallowed_color, axis=1))
                else False
            )
            ...
            if component_mask_combined is None:
                ...
                if component_mask.shape != component_mask_combined.shape:
                    component_mask = cv2.resize(
                        component_mask,
                        (component_mask_combined.shape[1], component_mask_combined.shape[0]),
                    )
...
def plot_interpolations(encoder, decoder, train_loader, num_pairs=5, num_steps=10):
    ...
    for pair_idx in range(num_pairs):
        ...
        interpolated_imgs = [
            decoder(z).squeeze(0).detach().cpu().numpy().transpose(1, 2, 0) * 0.5 + 0.5
...
def plot_intermedidate_figs(
    ...
    plt.savefig(
        f"{video_savepath_part}/{component}_inter_anomaly_map_{idx}.png",
        dpi=300,
        bbox_inches="tight",
    )
    if destroy_figs:
...
def plot_label_timeline_dual(loader_or_dataset, fps=5, title="Label Timeline (Frames + Time)"):
    ...
    axes[0].imshow(gt_img, aspect="auto", cmap=cmap, extent=[0, n - 1 if n > 1 else 1, 0, 1])
    ...
    axes[1].imshow(
        gt_img, aspect="auto", cmap=cmap, extent=[times[0], times[-1] if n > 1 else 1 / fps, 0, 1]
    )
...
def plot_label_timeline_dual_axis(
    loader_or_dataset, fps=5, title="Anomaly Timeline (Frame + Time)"
):
