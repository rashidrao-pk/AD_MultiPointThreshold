...
def _distance_offset_np(imgA: np.ndarray, imgB: np.ndarray, offset: int) -> np.ndarray:
    ...
    for di in range(-offset, offset + 1):
        for dj in range(-offset, offset + 1):
            ...
            j0b = max(0, -dj)
            j1b = min(W, W - dj)
            d = np.sqrt(((imgA[i0a:i1a, j0a:j1a] - imgB[i0b:i1b, j0b:j1b]) ** 2).sum(axis=2))
            dist[i0a:i1a, j0a:j1a] = np.minimum(dist[i0a:i1a, j0a:j1a], d.astype(np.float32))
    return dist


def score_pair(
    imgA: np.ndarray, imgB: np.ndarray, offset: int, sigma: float, quantile: float
...
def score_batch(
    data_t: torch.Tensor, recon_t: torch.Tensor, offset: int, sigma: float, quantile: float
) -> np.ndarray:
...
def load_val_loader(split_json: str, root: str, batch_size: int, num_workers: int) -> tuple:
    ...
    loader = DataLoader(
        sub_ds, batch_size=batch_size, shuffle=False, num_workers=num_workers, drop_last=False
    )
    return loader, sub_ds, info["val_indices"]


def load_test_loader(test_dir: str, batch_size: int, num_workers: int) -> tuple:
...
def run_val_mode(area: str, args, device, out_dir: str) -> dict:
    ...
    split_json = os.path.join(paths.train_dir_processed_subgroup, f"split_4train_1val_{area}.json")
    ...
    val_loader, val_sub, val_indices = load_val_loader(
        split_json, paths.train_dir_processed_subgroup, args.batch_size, args.num_workers
    )
    ...
    score_csv = os.path.join(
        out_dir, area, f"val_scores_{area}_off{args.offset}_sig{args.sigma}_q{args.quantile}.csv"
    )
...
def _compute_scores_for_loader(
    loader, dataset, Enc, Dec, score_fn, good_classname: str, gt_frame_set: set, device
...
def _evaluate_method(
    name, scores, labels, threshold, monitor_score, current_best
...
def _save_calibration_plots(
    name, scores, labels, threshold, params, save_dir: str, destroy: bool = True
):
    ...
    for lbl, (idxs, color) in cat_data.items():
        if len(idxs):
            ax1.scatter(
                idxs, scores[idxs], label=f"{lbl} ({len(idxs)})", alpha=0.6, color=color, s=12
            )
    ax1.axhline(threshold, color="gray", linestyle="--", label=f"tau = {threshold:.4f}")
    ax1.set_title(
    ...
    if len(tnv):
        ax2.axvline(tnv.mean(), color="blue", ls="--", label=f"TN mean={tnv.mean():.3f}")
    if len(tpv):
        ax2.axvline(tpv.mean(), color="green", ls="--", label=f"TP mean={tpv.mean():.3f}")
    handles, lbl_names = ax2.get_legend_handles_labels()
    handles.append(Line2D([0], [0], color="none", label=f"bAUC: {b_auc:.4f}"))
    ...
    fig.savefig(
        os.path.join(save_dir, f"{name.replace(' ', '_')}_{params.subgroup}_plot.png"),
        dpi=120,
        bbox_inches="tight",
    )
    if destroy:
...
def run_test_mode(area: str, args, device, out_dir: str) -> dict:
    ...
    test_loader, test_ds = load_test_loader(area_test_dir, args.batch_size, args.num_workers)
    ...
    for fn_idx, (name, score_fn) in enumerate(
        ...
        scores, labels, fnames = _compute_scores_for_loader(
            test_loader, test_ds, Enc, Dec, score_fn, good_classname, gt_frame_set, device
        )
        ...
        metrics, best_score, is_best, threshold = _evaluate_method(
            name, scores, labels, threshold, args.monitor_score, best_score
        )
        if is_best:
            best_idx = fn_idx
        ...
        _save_calibration_plots(name, scores, labels, threshold, params, plot_dir, destroy=True)

    if best_name is None:
        raise RuntimeError("No valid scoring method found (check GT labels).")

    # ── Rename best-method plot ───────────────────────────────────────────
    old_f = os.path.join(plot_dir, f"{best_name.replace(' ', '_')}_{area}_plot.png")
    new_f = os.path.join(plot_dir, f"BEST-{best_name.replace(' ', '_')}_{area}_plot.png")
    ...
    scores_b, labels_b, fnames_b = _compute_scores_for_loader(
        test_loader, test_ds, Enc, Dec, best_fn, good_classname, gt_frame_set, device
    )
    df_scores = pd.DataFrame({"file_name": fnames_b, "anomaly_score": scores_b, "label": labels_b})
    score_csv = os.path.join(area_out, f"test_scores_{area}_{best_name.replace(' ', '_')}.csv")
    df_scores.to_csv(score_csv, index=False)

    # ── Metrics table ─────────────────────────────────────────────────────
    df_metrics = pd.DataFrame(all_metrics).sort_values("binormal_AUC", ascending=False)
...
def _build_summary(area, suffix, n_epochs, args, tau, df_scores, score_csv, mode, **extra) -> dict:
...
def parse_args():
    ...
    p.add_argument(
        "--safety_area", default="RoboArm", help="Area to calibrate. 'ALL' processes all areas."
    )
    ...
    p.add_argument(
        "--test_folder",
        default=r"v2/ camera1_20251210_151444_fallen_operator/test/operator_fall",
    ...
    p.add_argument(
        ...
        choices=["scripts/checkpoints_33", "scripts/results"],
    )
    p.add_argument(
        "--gt_csv",
        default="scripts/data/annotations/anom_metadata_operator_fall.csv",
    ...
    p.add_argument(
        "--offset_ls", default="1,2,3", help="[test mode] Comma-separated offsets to sweep."
    )
    p.add_argument(
        "--quantile_ls",
        default="1.0,0.999,0.99",
        help="[test mode] Comma-separated quantiles to sweep.",
    )
    p.add_argument(
        "--sigma_ls", default="0,0.5,1.0,1.5", help="[test mode] Comma-separated sigmas to sweep."
    )
    p.add_argument(
        "--threshold_method",
        default="f1c",
...
def main():
    ...
    paths.path_codes_main = os.path.join(paths.path_codes, "scripts")
    paths.path_models = os.path.join(os.getcwd(), args.checkpoints)
    out_dir = args.output_dir or os.path.join(paths.path_codes_main, "results", "thresholds")
    ...
    if args.verbose_level > 1:
        ...
        print(
            f"TEst Folder FILE WILL BE LOADED FROM \t {os.path.exists(args.test_dir)} - {args.test_dir}"
        )
        print(
            f"CSV FILE WILL BE LOADED FROM \t{os.path.exists(args.gt_csv_path)} - {args.gt_csv_path}"
        )
        print("-" * 100)
    ...
    if all_summaries:
        summary_csv = os.path.join(
            out_dir, f"thresholds_summary_{args.mode}_{args.threshold_strategy}.csv"
        )
