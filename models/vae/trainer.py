...
def _reconstruction_loss(name):
    """Return the reconstruction loss function selected for the VAE baseline."""
    key = str(name or "MSE").lower()
    if key in {"l1", "mae"}:
        return nn.L1Loss()
    if key == "mse":
        return nn.MSELoss()
    if key in {"bce", "bcewithlogits"}:
        return nn.MSELoss()
    raise ValueError(f"Unsupported VAE reconstruction loss: {name}")


def train_one_epoch(encoder, decoder, train_loader, optimizer, device, loss_fn, beta_kl=1e-4):
    """Train the vanilla VAE for one epoch and return averaged metrics."""
    encoder.train()
    decoder.train()
    totals = {
        "recon_loss": 0.0,
    ...
    for images, _ in tqdm(train_loader, desc="train batches", leave=False):
        ...
        vae_loss = recon_loss + beta_kl * kl_loss
        vae_loss.backward()
        optimizer.step()
...
def validate(encoder, decoder, val_loader, device, loss_fn):
    """Compute validation reconstruction loss using latent means."""
    encoder.eval()
    decoder.eval()
    total = 0.0
    n_batches = 0
...
def train_model(
    ...
    """Train a vanilla VAE and save compatible checkpoints, curves, and previews."""
    ...
    for epoch in tqdm(range(1, epochs + 1), desc="epochs"):
        ...
        if epoch % save_every == 0 or epoch == epochs:
            save_checkpoint(
                ...
                notes="Latest vanilla VAE checkpoint for this training run.",
            )
        ...
        if val_metrics["val_recon_loss"] < best_val_loss:
            ...
            save_checkpoint(
                ...
                metrics=epoch_metrics,
                model_name=model_name,
                notes="Best vanilla VAE checkpoint by validation reconstruction loss.",
            )
